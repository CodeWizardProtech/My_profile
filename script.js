...
function initMobileMenu() {
  ...
  const closeMenu = () => {
    ...
    setTimeout(() => {
      if (!navMenu.classList.contains("active") && isMobileView()) {
  ...
  // Toggle menu no clique do hambúrguer
      ...
      fetch(form.action, {
        ...
        .then((response) => {
          ...
          if (response.ok) {
            ...
            showStatusMessage(
              false,
              response.text || "Erro no servidor. Código: " + response.status
            );
          }
        })
        .catch((error) => {
